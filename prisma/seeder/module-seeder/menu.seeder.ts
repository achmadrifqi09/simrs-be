...
const readJsonFile = <T>(filename: string): T[] => {
  const result = fs.readFileSync(
    path.resolve(__dirname, `../data/menu/${filename}`),
    'utf8',
  );
...
export const menuSeeder = async () => {
  try {
    console.log('Seeding menu ...');
    const menus = readJsonFile<Country>('menu.json');
    await batchCreate(prismaClient.menu, menus);

    console.log('Seeding menu permission ...');
    const permission = readJsonFile<Country>('menu_permission.json');
    await batchCreate(prismaClient.menuPermission, permission);
  } catch (error) {
